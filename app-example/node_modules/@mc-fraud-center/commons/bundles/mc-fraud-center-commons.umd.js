(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@angular/core'), require('@angular/forms'), require('moment'), require('@ngx-translate/core'), require('@mastercard/ng-commons'), require('rxjs/operators/tap'), require('ngx-bootstrap'), require('rxjs/Subject'), require('ngx-toastr'), require('cerialize'), require('@angular/http'), require('lodash'), require('rxjs/observable/throw'), require('rxjs/observable/of'), require('rxjs/operators'), require('rxjs/operators/filter'), require('rxjs/operators/map'), require('@angular/common')) :
	typeof define === 'function' && define.amd ? define(['exports', '@angular/core', '@angular/forms', 'moment', '@ngx-translate/core', '@mastercard/ng-commons', 'rxjs/operators/tap', 'ngx-bootstrap', 'rxjs/Subject', 'ngx-toastr', 'cerialize', '@angular/http', 'lodash', 'rxjs/observable/throw', 'rxjs/observable/of', 'rxjs/operators', 'rxjs/operators/filter', 'rxjs/operators/map', '@angular/common'], factory) :
	(factory((global['mc-fraud-center'] = global['mc-fraud-center'] || {}, global['mc-fraud-center'].commons = {}),global.ng.core,global.ng.forms,global.moment,global['@ngx-translate/core'],global['@mastercard/ng-commons'],global.Rx.Observable.prototype,global['ngx-bootstrap'],global.Rx,global['ngx-toastr'],global.cerialize,global.ng.http,global.lodash,global.Rx.Observable,global.Rx.Observable,global.Rx.Observable.prototype,global.Rx.Observable.prototype,global.Rx.Observable.prototype,global.ng.common));
}(this, (function (exports,core,forms,momentNS,core$1,ngCommons,tap,ngxBootstrap,Subject,ngxToastr,cerialize,http,lodash,_throw,of,operators,filter,map,common) { 'use strict';
...
DatePickerComponent.propDecorators = {
    ...
    "additionalFormats": [{ type: core.Input },],
    "min": [{ type: core.Input },],
    "max": [{ type: core.Input },],
    "allowInvalid": [{ type: core.Input },],
    "allowEdit": [{ type: core.Input },],
    "customClassNames": [{ type: core.Input },],
...
var DownloadButtonComponent = /** @class */ (function () {
    ...
    DownloadButtonComponent.prototype.save = function (download$) {
        var _this = this;
        return download$.pipe(tap.tap(function (res) {
...
DownloadButtonComponent.propDecorators = {
    ...
    "downloadAnchor": [{ type: core.ViewChild, args: ['downloadAnchor',] },],
};
...
FcDialogComponent.propDecorators = {
    "modal": [{ type: core.ViewChild, args: [ngxBootstrap.ModalDirective,] },],
    "titleDialog": [{ type: core.Input },],
    "labelDialog": [{ type: core.Input },],
    "labelSecondaryButton": [{ type: core.Input },],
    "labelPrimaryButton": [{ type: core.Input },],
    "buttonClick": [{ type: core.Output },],
    "enableSecondaryButton": [{ type: core.Input },],
};
...
__decorate([
    cerialize.deserializeAs(ErrorObject),
...
var CacheableHttpService = /** @class */ (function () {
    ...
    CacheableHttpService.prototype.get = function (url, options, expiresIn, forceRefresh) {
        ...
        return of.of(this.cache[url]).pipe(operators.mergeMap(function (cache) {
            if (cache && forceRefresh)
                cache.expiresIn = 0;
            if (cache && lodash.isEqual(options, cache.options) && !_this.isExpired(cache.expiresIn)) {
                _this.logger.debug('from cache', url);
                return cache.stream;
            }
            else {
                _this.logger.debug('caching', url);
                var stream = _this.delegate.get(url, options).pipe(operators.shareReplay(1));
                if (expiresIn >= 0)
                    expiresIn = expiresIn * 1000 + new Date().getTime();
                _this.cache[url] = { stream: stream, options: options, expiresIn: expiresIn };
                return stream;
            }
        }), operators.tap(function () { return _this.clearStaleEntries(); }), operators.catchError(function (error) {
    ...
    CacheableHttpService.prototype.clearStaleEntries = function () {
        ...
        this.cache = (lodash.pickBy(this.cache, function (item) { return !_this.isExpired(item.expiresIn); }));
...
var HttpService = /** @class */ (function () {
    ...
    Object.defineProperty(HttpService.prototype, "defaultOptions", {
        get: function () {
            return new http.RequestOptions({ headers: this.defaultHeaders });
        },
        enumerable: true,
        configurable: true
    });
    ...
    HttpService.prototype.serialize = function (request) {
        return request.requestType ? cerialize.Serialize(request.body, request.requestType) : request.body;
    };
    HttpService.prototype.handle = function (request, response$) {
        var _this = this;
        var waitMessage;
        response$ = of.of(request.waitMessage).pipe(operators.mergeMap(function (message) {
            if (typeof message === 'string') {
                return _this.translate.get(message).pipe(operators.tap(function (translated) {
            ...
            return of.of(false);
        }), operators.mergeMapTo(response$));
        return this.handleResponseStatus(request, response$).pipe(operators.map(function (response) { return _this.deserialize(request, response); }), operators.catchError(function (error) { return _this.handleError(request, error); }), operators.mergeMap(function (response) { return _this.handleSuccess(request, response); }), operators.finalize(function () {
            if (waitMessage) {
    ...
    HttpService.prototype.handleResponseStatus = function (request, response$) {
        ...
        var handle = function (response) {
            ...
            var payload;
            try {
                payload = cerialize.Deserialize(response.json(), ErrorResponse);
        ...
        return response$.pipe(operators.catchError(handle), operators.mergeMap(handle));
    };
    HttpService.prototype.deserialize = function (request, response) {
        if (request.responseType) {
            var data = cerialize.Deserialize(response.json(), request.responseType);
    ...
    HttpService.prototype.handleError = function (request, error) {
        ...
        return this.translate.get(result.displayMessage).pipe(operators.mergeMap(function (translated) {
    ...
    HttpService.prototype.handleSuccess = function (request, response) {
        ...
        if (request.toastOnSuccess) {
            return this.translate.get(token).pipe(operators.map(function (translated) {
...
var PermissionsService = /** @class */ (function () {
    ...
    PermissionsService.prototype.init = function () {
        ...
        this.stream = this.messageService.get().pipe(operators.filter(function (me) { return me.data.message === MessageKey.GET_ACCESSIBLE_UI_ACTIONS_RESPONSE; }), operators.map(function (me) { return (me.data.content); }), operators.publishReplay(1), operators.refCount());
    ...
    PermissionsService.prototype.hasActionCode = function (actionCode) {
        return this.availableActions.pipe(operators.map(function (actions) { return actions.some(function (a) { return a.code === actionCode; }); }));
    };
    PermissionsService.prototype.hasSome = function () {
        ...
        return this.availableActions.pipe(operators.map(function (actions) { return actionCodes.some(function (requested) { return actions.find(function (a) { return a.code === requested; }) !== undefined; }); }));
    };
    PermissionsService.prototype.hasEvery = function () {
        ...
        return this.availableActions.pipe(operators.map(function (actions) { return actionCodes.every(function (requested) { return actions.find(function (a) { return a.code === requested; }) !== undefined; }); }));
    };
...
var PleaseWaitComponent = /** @class */ (function () {
    function PleaseWaitComponent(service, logger) {
        ...
        this.messages = [];
        this.trackByIndex = ngCommons.trackByIndex;
    }
...
var PreventNavigationService = /** @class */ (function () {
    ...
    PreventNavigationService.prototype.start = function () {
        ...
        this.messageService.get().pipe(filter.filter(function (incoming) { return incoming.data.message === MessageKeys.CONFIRMATION_DIALOG_OPENED; })).subscribe(function () {
    ...
    PreventNavigationService.prototype.confirmationDialogClosed = function () {
        return this.messageService.get().pipe(filter.filter(function (incoming) { return incoming.data.message === MessageKeys.CONFIRMATION_DIALOG_CLOSED; }), map.map(function (incoming) { return (incoming.data.content.ok); }));
    };
...
function httpFactory(http$$1, window, locale, logger, wait, translate, toastr) {
    return new HttpService(http$$1, window, locale, logger, wait, translate, toastr);
}
function xsrfStrategyFactory() {
    return new http.CookieXSRFStrategy('TFC-XSRF-Token', 'X-TFC-XSRF-Token');
}
        ...
        return {
            ...
            providers: [
                ...
                PreventNavigationService,
                {
                    ...
                    deps: [
                        CacheableHttpService, 'Window', ngCommons.LocaleService, ngCommons.Logger, PleaseWaitService, core$1.TranslateService, ToastrService$1
                    ],
                },
                {
                    provide: http.XSRFStrategy,
...
exports.FcDialogComponent = FcDialogComponent;
exports.HttpService = HttpService;
exports.HttpReadRequest = HttpReadRequest;
exports.HttpWriteRequest = HttpWriteRequest;
exports.HttpResponse = HttpResponse;
exports.HttpError = HttpError;
exports.HttpDisplayMessages = HttpDisplayMessages;
exports.HttpLogMessages = HttpLogMessages;
exports.CacheableHttpService = CacheableHttpService;
exports.ErrorObject = ErrorObject;
exports.ErrorResponse = ErrorResponse;
exports.PermissionsService = PermissionsService;
exports.PleaseWaitService = PleaseWaitService;
exports.PleaseWaitComponent = PleaseWaitComponent;
exports.MessageKeys = MessageKeys;
exports.PreventNavigationService = PreventNavigationService;
exports.ToastrService = ToastrService$1;
exports.httpFactory = httpFactory;
exports.xsrfStrategyFactory = xsrfStrategyFactory;
exports.FraudCenterCommons = FraudCenterCommons;

Object.defineProperty(exports, '__esModule', { value: true });

})));
//# sourceMappingURL=mc-fraud-center-commons.umd.js.map
